...
public class Library
{
    ...
    private void add(ref Grid grid, int row, int column)
    {
        ...
        canvas.Tapped += (object sender, TappedRoutedEventArgs e) =>
        {
            if (!won)
            {
                ...
                if (canvas.Children.Count < 1) // this canvas hasn't been clicked. If clicked there will be a path in its children.
                {
    ...
    public async void New(Grid grid)
    {
        ...
        board[2, 2] = "";
        won = false;
        if (await Confirm("Who goes First?", "Noughts and Crossses", nought, cross))
        {
            piece = nought;
        }
        else
